...
<script type="text/javascript">
	...
	function updateAccount() {
		var taskID = $("#taskID").val();
		//window.location.href = "pages/personal/updateBankAccount.jsp?taskID="+taskID+"&processType=payment";
		window.location.href = "/finance/reimbursement/updateBankAccount?taskID="+taskID+"&processType=payment&bankAccountId=${reimbursementVO.bankAccountId}";
		Load.Base.LoadingPic.FullScreenShow(null);
	}
      ...
      <div class="row">
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
        	<form class="form-horizontal">
        	  <h3 class="sub-header" style="margin-top:0px;">付款申请审批</h3>
        	  <input type="hidden" id="taskID" value="${taskID}"/>
			  
			    <div class="Cost">
				<div class="Costlm"><span>付 款 申 请 单</span></div>
				<div class="unit"><span class="fl">申请部门：<em>${departmentName }（${companyName }）</em></span><span class="fl">付款单号：<em>${reimbursementVO.reimbursementNo }</em></span><span class="fr">申请日期：<em>${reimbursementVO.requestDate.split(" ")[0] }</em></span></div>
				<div class="Cost_tab">
...
<%-- 				      	<c:if test="${taskDefKey == 'financialFirstAudit' }">同意</c:if> --%>
				...
				<div class="Cost_foot"><span>会计主管：<c:if test="${taskDefKey ne 'supervisor_audit' and taskDefKey ne 'manager_audit' and taskDefKey ne 'financialFirstAudit'}"><em>${reimbursementVO.showPerson2}</em></c:if></span><span>会计：<em>${reimbursementVO.showPerson1}</em></span><span>出纳：<em>${reimbursementVO.showPerson3}</em></span><span>申请人：<em>${reimbursementVO.requestUserName }</em></span><span>领款人：<em>${reimbursementVO.payeeName }</em></span></div>				
