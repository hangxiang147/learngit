	...
	<div class="container-fluid">
      <div class="row">
      		...
      		<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
      			...
      			action="/performance/soft/save_saveSingleTask" method="post" onsubmit="Load.Base.LoadingPic.FullScreenShow(null)">
      			<div class="form-group">
      			<s:token></s:token>
      			<label for="project" class="col-sm-1 control-label">所属项目</label>
      			<div id="project" class="col-sm-2">
      				<select class="form-control" required id="selectProject" disabled name="softPerformanceVo.projectId" onchange="changeProject(),changeName(this)">
					...
					<input type="hidden" name="softPerformanceVo.projectName">
      			</div>
      			<label for="version" class="col-sm-1 control-label">所属版本</label>
      				<div id="version" class="col-sm-2">
      				<select class="form-control" disabled required id="selectVersion" name='softPerformanceVo.projectVersionId' onchange="changeProjectVersionOrModule(),changeName(this)">
					...
					<input type="hidden" name="softPerformanceVo.versionName">
					</div>
      			<label for="module" class="col-sm-1 control-label">所属模块</label>
      				<div id="module" class="col-sm-2">
      				<select class="form-control" disabled required id="selectModule" onchange="changeProjectVersionOrModule(),changeName(this)" name='softPerformanceVo.moduleId'>
      					<c:forEach items="${modules}" var="module">
      						<option value="${module.id}" <c:if test="${softPerformanceVo.moduleId==module.id}">selected="selected"</c:if>>${module.module}
      						</option>
      					</c:forEach>
					</select>
					<input type="hidden" name="softPerformanceVo.moduleName">
					</div>
      			</div>
<%-- 				<div class="form-group">
				<label for="associatedRequire" class="col-sm-1 control-label">关联需求</label>
					...
					<!-- <div class="btn btn-primary" onclick="showSubRequire()" style="margin-left:2%">查看子需求</div> -->
      			</div> --%>
      			<div class="form-group">
      				<label for="name" class="col-sm-1 control-label">任务名称</label>
      				<div id="name" class="col-sm-5">
						<input name="softPerformanceVo.name" disabled autocomplete="off" required="required" value="${softPerformanceVo.name}" class="form-control"/>
					</div>
					<label for="score" class="col-sm-1 control-label">分值</label>
      				<div id="score" class="col-sm-2">
						<input required="required" disabled autocomplete="off" type="number" min=0 max=100 name="softPerformanceVo.score" value="${softPerformanceVo.score}" class="form-control"/>
					</div>
      			</div>
      			<div class="form-group">
      			<label for="assigner" class="col-sm-1 control-label">指派</label>
					<div id="assigner" class="col-sm-2 inputout">
						...
						<input id="assignerName" autocomplete="off" class="form-control" required name="softPerformanceVo.assignerName"
				...
				<label for="assigner" class="col-sm-1 control-label">截止时间</label>
					<div class="col-sm-2">
					<input type="text" autocomplete="off" class="form-control" name="softPerformanceVo.deadline"
			    		onclick="WdatePicker({ dateFmt: 'yyyy-MM-dd' ,minDate:'%y-%M-%d'})" placeholder="截止时间">
			    	</div>
				</div>
      			<div class="form-group">
					<label for="description" class="col-sm-1 control-label">任务描述</label>
					<div id="description" class="col-sm-8">
						<textarea class="form-control" name="softPerformanceVo.description" data-provide="markdown" rows="7"></textarea>
					</div>
				</div>
				<div class="form-group">
...
<%-- 			  	<div class="form-group">
			  	...
			  	<div class="col-sm-8">
			  		<input type="radio" name="nextStep" value="singleTask" style="height:15px;width:15px">&nbsp;<label>继续为该需求添加任务</label>
			  		<input type="radio" name="nextStep" value="render_showTask" checked style="height:15px;width:15px">&nbsp;<label>返回任务列表</label>
			  		<input type="radio" name="nextStep" value="render_showRequirement" style="height:15px;width:15px">&nbsp;<label>返回需求列表</label>
			  	</div>
				</div> --%>
      			<div class="form-group">
			    ...
			    <button type="button" class="btn btn-default" onclick="javascript:location.href='javascript:history.go(-1);'" style="margin-left:2%">返回</button>
					...
					<input type="hidden" name="loginUserId" value="<%=((User) request.getSession().getAttribute("user")).getId() %>" />
      				<input type="hidden" name="softPerformanceVo.requirementId" value="${requirementId}" />
      				<input type="hidden" name="softPerformanceVo.subRequirementId" value="${subRequirementId}" />
      				<input type="hidden" name="softPerformanceVo.requirementName" value="${softPerformanceVo.requirementName}" />
      				<input type="hidden" name="attachmentPathStr" value="${softPerformanceVo.attachmentPath}"/>
					<input type="hidden" name="attachmentNameStr" value="${softPerformanceVo.attachmentName}"/>
      			</form>
      		</div>
      </div>
    </div>
    <%@include file="/pages/performance/soft/subject/showSubRequire.jsp" %>
	...
	<script type="text/javascript">
    	$(function(){
        	...
        	setName("selectProject");
        	setName("selectVersion");
        	setName("selectModule");
       ...
       $("#saveRequirementInfo").submit(function(){
    	   ...
    	   $("input[name='softPerformanceVo.name']").removeAttr("disabled");
    	   $("input[name='softPerformanceVo.score']").removeAttr("disabled");
       ...
       function setName(id){
    	   var item = $("#"+id).find("option:selected").text();
    	   $("#"+id).next().val(item);
       }
       function showSubRequire(){
    	   var requirementId = $("#selectAssociatedRequire option:selected").val();
    	   $(".modal").modal('show');
    	   $.ajax({
    		   	url:'/performance/soft/showSubRequirementLst',
      			type:'post',
      			data:{'requirementId':requirementId},
      			dataType:'json',
      			success:function (data){
      				var subRequirementLst = data.subRequirementLst;
      				var html = '';
      				for(var i=0; i<subRequirementLst.length; i++){
      					html += '<tr><td class="left">'+(i+1)+'</td>'+
								'<td class="left">'+subRequirementLst[i].subRequirementName+'</td>'+
								'<td class="description left">'+subRequirementLst[i].description+'</td>';
								var attachmentPaths = subRequirementLst[i].attachmentPaths;
								var attachmentNames = subRequirementLst[i].attachmentNames;
								var attachmentHtml = '<td class="left">';
								if(attachmentPaths!=undefined){
									for(var j=0; j<attachmentPaths.length; j++){
										attachmentHtml += '<a href="/performance/soft/downloadAttachment?attachmentPath='
												+attachmentPaths[j]+'&attachmentName='+attachmentNames[j]+'">'+attachmentNames[j]+'</a><br>';
									}
								}
								attachmentHtml += '</td>';
								html += attachmentHtml;
   	...
   	function deleteAttachment(target){
   		var index = layer.open({  
            ...
            yes: function() {  
        		...
        		var subRequirementId = $("input[name='softPerformanceVo.subRequirementId']").val();
        		$.ajax({
        			type:'post',
        			data:{'attachmentName':attachmentName,'attachmentPath':attachmentPath,'subRequireId':subRequirementId},
