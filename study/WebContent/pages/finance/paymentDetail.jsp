...
<script type="text/javascript">
	//解决 一些浏览器 上 print 之后 无参数 页面报错问题
...
<body>
	<div class="container-fluid">
      <div class="row">
      	<%-- <c:if test="${panel!='personal'}">
      		 <c:if test="${panel=='reimbursement'}"><s:set name="panel" value="'reimbursement'"></s:set></c:if>
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
        	<form class="form-horizontal">
        	  <h3 class="sub-header" style="margin-top:0px;">付款单详情</h3>
			    <div class="Cost">
        		<div id="printArea">
				<div class="Costlm"><span>付 款 申 请 单</span></div>
				<div class="unit"><span class="fl">申请部门：<em>${departmentName }（${companyName }）</em></span><span class="fl">付款单号：<em>${reimbursementVO.reimbursementNo }</em></span><span class="fr">付款日期：<em>${reimbursementVO.requestDate.split(" ")[0] }</em></span></div>
				<div class="Cost_tab">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
				    <tr>
				      <td width="46%" align="center" class="col_blue">用途</td>
				      <td width="20%" align="center" class="col_blue">金额（元）</td>
				...
				<div class="Cost_foot"><span>会计主管：<em>${reimbursementVO.showPerson2}</em></span><span>会计：<em>${reimbursementVO.showPerson1}</em></span><span>出纳：<em>${reimbursementVO.showPerson3}</em></span><span>申请人：<em>${reimbursementVO.requestUserName }</em></span><span>领款人：<em>${reimbursementVO.payeeName }</em></span></div>				
				</div>
				...
				<table>
					<c:forEach items="${attas}" var="item" varStatus="index">
						 <c:if test="${item.type eq 'picture'}">
							<a href="javascript:showPic(${index.index })" target="_self"><img style="height:200px;width:200px;margin-left:20px" src="personal/getVacationAttachmentAll_?processInstanceID=${processInstanceID}&index=${index.index}"/>
							</a>						 
						 </c:if>
						 <c:if test="${item.type ne 'picture' }">
							 <a	href="personal/getVacationAttachmentAll_?processInstanceID=${processInstanceID}&index=${index.index}" >${item.name}</a>
						 </c:if>
					</c:forEach>
				</table>
				</div>
    ...
    <script src="/assets/js/layer/layer.js"></script>
   	...
   	<c:forEach items="${attas}" var="item" varStatus="index">
   	//附件 仅仅有图片 
	 	picData.data.push({alt:"${item.name}",src:"/personal/getVacationAttachmentAll_?processInstanceID=${processInstanceID}&index=${index.index}"})
	</c:forEach>
