        ...
        <%@include file="/pages/informationCenter/panel.jsp" %>
			<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
				...
				<form  id="versionInfo" action="/information/version/save_saveVersionInfo"
					...
					<s:token></s:token>
					<input name="versionFuncionInfo.id" value="${versionFuncionInfo.id}" type="hidden">
					<input name="versionFuncionInfo.addTime" value="${versionFuncionInfo.addTime}" type="hidden">
					<input name="versionFuncionInfo.operator" value="${versionFuncionInfo.operator}" type="hidden">
					<input name="versionFuncionInfo.isDeleted" value="${versionFuncionInfo.isDeleted}" type="hidden">
					<input name="versionFuncionInfo.versionDate" type="hidden" value="${versionFuncionInfo.versionDate}">
					...
					<select id="versionName" class="form-control" required name="versionFuncionInfo.versionName" onchange="changeVersion()">
						...
						<c:forEach items="${versions}" var="version">
							<option ${versionFuncionInfo.versionName==version.version?'selected':''} data-versionDate="${version.endDate}" value="${version.version}">${version.version}</option>
						</c:forEach>
					</select>
					</div>
					</div>
					<div class="form-group">
						...
						<div class="col-sm-5">
						<textarea rows="3" class="form-control" name="versionFuncionInfo.function" required>${versionFuncionInfo.function[0]}</textarea>
						</div>
						<button type="button" class="btn btn-primary" onclick="addFunction()">新增</button>
					</div>
					<c:if test="${fn:length(versionFuncionInfo.function)>1}">
						<c:forEach items="${versionFuncionInfo.function}" var="item" begin="1">
							<div class="form-group">
							<div class="col-sm-1" style="width:12%"></div>
							<div class="col-sm-5">
							<textarea rows="3" class="form-control" name="versionFuncionInfo.function" required>${item}</textarea>
							</div>
