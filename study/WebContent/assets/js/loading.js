...
var Load = {};
Load.Base = {};
Load.Base.LoadingPic = {
    operation: {
        timeTest: null,                     //延时器
        loadingCount: 0,                    //计数器 当同时被多次调用的时候 记录次数
        loadingImgUrl: "/assets/images/loading.gif",  //默认load图地址
    ...
    FullScreenShow: function (msg) {
        ...
        if (this.operation.loadingCount < 1) {
            this.operation.timeTest = setTimeout(function () {
                ...
                $("#loadingPage_bg").height($(document).height()).width($(top.window.document).width());
            }, 100);
        }
