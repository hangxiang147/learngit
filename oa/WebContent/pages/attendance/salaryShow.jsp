...
.float_div{
  ...
  <body>
    ...
    <div class="container-fluid">
      <div class="row"> 
      	<s:set name="selectedPanel" value="'salaryShow'"></s:set>
        <%@include file="/pages/attendance/salayPanel.jsp" %>		
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
          <form  id="form_" class="form-horizontal" action="/attendance/salaryShow" onsubmit="Load.Base.LoadingPic.FullScreenShow(null)">     	  
          	<h3 class="sub-header" style="margin-top:0px;">工资单查看</h3> 
			   <div class="form-group" >
			  	<label for="reason" class="col-sm-1 control-label">姓名</label>
			  	<div class="col-sm-2" ><input type="text" class="form-control" name="name" value="${name}"></div>
<!-- 			  	<label  class="col-sm-1 control-label">状态</label> -->
<!-- 			  	<div class="col-sm-2" > -->
<%-- 			  		<select name="type" class="form-control"> --%>
<!-- 			  			<option value="">--所有--</option> -->
<%-- 			  			<option value="-1" ${type eq '-1'?"selected":"" }>待确认</option> --%>
<%-- 			  			<option value="2" ${type eq '2'?"selected":"" }>已确认</option> --%>
<%-- 			  			<option value="1" ${type eq '1'?"selected":"" }>需要人事重新审核</option> --%>
<%-- 			  		</select> --%>
<!-- 			  	</div> -->
			...
			<div class="form-group">
				...
				<div class="col-sm-2">
					<select name ="mobileSend"  class="form-control"  >
						<option value="">请选择</option>
						<option value="1" ${mobileSend eq '1'?"selected":""}>已推送</option>
						<option value="-1" ${mobileSend eq '-1'?"selected":""}>未推送</option>
					</select>
				</div>
				...
				<div class="col-sm-2">
					<select name ="emailSend"  class="form-control"  >
						<option value="">请选择</option>
						<option value="1" ${emailSend eq '1'?"selected":""} >已推送</option>
						<option value="-1" ${emailSend eq '1'?"selected":""} >未推送</option>
					</select>
				</div>
...
<%--               		<td>${item.content }</td> --%>
				<td>
				<a href="javascript:showSalary('${item.detailList}','${item.status}','${item.year}','${item.month}','${item.departmentName}','${item.name}')">
