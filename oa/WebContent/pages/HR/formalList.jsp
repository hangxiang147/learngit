              ...
              <tbody>
              	...
              	<s:iterator id="taskVO" value="#request.taskVOs" status="count">
              		<tr>
              			...
              			<td class="col-sm-1">
              			<s:if test="#taskVO.taskDefKey == 'formalInvitation'">
              			<a onclick="goPath('HR/process/sendInvitation?taskID=<s:property value='#taskVO.taskID'/>')" href="javascript:void(0)">
              			<svg class="icon" aria-hidden="true" title="发送邀请" data-toggle="tooltip">
						<use xlink:href="#icon-banli"></use>
						</svg>
              			</a>
              			&nbsp;
              			<a onclick="goPath('HR/process/taskComplete?taskID=<s:property value='#taskVO.taskID'/>&result=9&businessType=转正申请')" href="javascript:void(0)">
                		<svg class="icon" aria-hidden="true" title="拒绝" data-toggle="tooltip">
						<use xlink:href="#icon-butongyi"></use>
						</svg>
              			</a>
              			</s:if>
              			<s:else>
              			<a onclick="goPath('HR/process/auditTask?taskID=<s:property value='#taskVO.taskID' />&selectedPanel=formalList')" href="javascript:void(0)">
