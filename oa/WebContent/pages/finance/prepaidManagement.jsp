...
function search(){
	window.location.href = "finance/reimbursement/prepaidManagement?" + $("#queryReimbursementForm").serialize();
...
<body>
	<div class="container-fluid">
		<div class="row">
			...
			<s:set name="selectedPanel" value="'prepaidManagement'"></s:set>
			...
			<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
				<form action="" id="queryReimbursementForm" class="form-horizontal">
					...
					<div class="form-group">
						<label for="demandName" class="col-sm-1 control-label">预付人</label>
						<div class="col-sm-2">
							<input type="text" class="form-control" id="requestUserName" name="advanceVo.requestUserName" value="${advanceVo.requestUserName }" >
						</div>
						
						<label for="reimbursementNo" class="col-sm-1 control-label">预付单号</label>
						<div class="col-sm-2">
							<input type="text" class="form-control" id="reimbursementNo" name="advanceVo.reimbursementNo" value="${advanceVo.reimbursementNo }" >
						</div>
						
						<label for="beginDate" class="col-sm-1 control-label">发起时间</label>
						<div class="col-sm-2">
							<input type="text" class="form-control" id="beginDate" name="advanceVo.beginDate" value="${advanceVo.beginDate }" onclick="WdatePicker({maxDate:'%y-%M-%d'})" placeholder="开始时间">
						</div>
						<div class="col-sm-2">
							<input type="text" class="form-control" id="endDate" name="advanceVo.endDate" value="${advanceVo.endDate }" onclick="WdatePicker({maxDate:'%y-%M-%d'})" placeholder="结束时间">
						</div>
					...
					<div class="form-group">
					    ...
					    <div class="col-sm-2">
					    	<select class="form-control" name="advanceVo.status" id="status">
					    	...
					    	<input id="statusEctype" type="hidden" class="form-control" value="${advanceVo.status }">
					    </div>
					    
					    <label for="" class="col-sm-1 control-label">当前环节</label>
					    <div class="col-sm-2">
					    	<select class="form-control" name="advanceVo.thecurrenLink" id="thecurrenLink">
								...
								<option value="上传发票">上传发票</option>
								<option value="审核发票">审核发票</option>
					    	</select>
					    	
					    	<input id="thecurrenLinkEctype" type="hidden" class="form-control" value="${advanceVo.thecurrenLink }">
					    </div>
						
						<label for="assigneeUserName" class="col-sm-1 control-label">待处理人</label>
						<div class="col-sm-2">
							<input id="assigneeUserName" type="text" class="form-control" name="advanceVo.assigneeUserName" value="${advanceVo.assigneeUserName }">
						</div>
					...
					<table class="table table-striped">
						<thead>
							<tr>
								...
								<th style="width:8%">预付人<br>/在职状态</th>
								<th style="width:12%">预付单号</th>
						...
						<tbody>
							<c:if test="${not empty advanceVoList }">
							<s:set name="reimbursement_id" value="(#request.page-1)*(#request.limit)"></s:set>
								<c:forEach items="${advanceVoList }" var="items" varStatus="number">
									<tr>
										...
										<td>
											<a onclick="goPath('finance/reimbursement/getAdvanceDetail?type=3&processInstanceID=${items.processInstanceID }')" href="javascript:void(0)">
						              			<svg class="icon" aria-hidden="true" title="查看预付单详情" data-toggle="tooltip">
						    						<use xlink:href="#icon-Detailedinquiry"></use>
												</svg>
					              			</a>
					              			&nbsp;
					              			<a onclick="goPath('finance/reimbursement/getReimbursementProcess?type=4&processInstanceID=${items.processInstanceID }&selectedPanel=${selectedPanel }')" href="javascript:void(0)">
