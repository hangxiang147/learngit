	...
	<div class="container-fluid">
      <div class="row">
      		...
      		<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
      			...
      			action="/performance/soft/save_saveRequirement" method="post">
      			<s:token></s:token>
      			<div class="form-group">
      			<label for="project" class="col-sm-1 control-label">所属项目</label>
      			<div id="project" class="col-sm-2">
      				<select class="form-control" required id="selectProject" onchange="changeProject()">
      			...
      			<label for="module" class="col-sm-1 control-label">所属模块</label>
      				...
      				<select class="form-control" required id="selectModule">
				...
				<label for="version" class="col-sm-1 control-label">所属版本</label>
      				...
      				<select class="form-control" required id="selectVersion">
      			...
      			<div id="sourse" class="col-sm-2">
      				<select class="form-control" required id="selectSourse">
					</select>
      			</div> --%>
				...
				<label for="remark" class="col-sm-1 control-label">备注</label>
					<div id="remark" class="col-sm-5">
      					<input name="requirementEntity.remark" autocomplete="off" value="${requirementEntity.remark}" class="form-control"/>
					</div>
      			</div>
      			<div class="form-group">
					...
					<label for="priority" class="col-sm-1 control-label">优先级</label>
      				<div id="priority" class="col-sm-2">
						<select class="form-control" required id="selectPriority">
...
<%--       			<div class="form-group">
      				<label for="reviewer" class="col-sm-1 control-label">评审人员</label>
      				<div id="reviewer" class="col-sm-2 inputout">
						<span class="input_text" onclick="f('reviewer')">
						<input id="reviewerName" class="form-control" required name="requirementEntity.reviewerName"
							value="${requirementEntity.reviewerName}" oninput="findStaffByName('reviewer')" onblur="clears('reviewer')"/>
				    	<input type="hidden" id="reviewerId" value="${requirementEntity.reviewerId}" name="requirementEntity.reviewerId" />
				    	<input type="hidden" id="reviewerFlag" value="${requirementEntity.reviewerName}"/>
				    	</span>
				    	<div class="reviewer_text_down text">
							<ul></ul>
						</div>
					</div>
      			</div> --%>
			    ...
			    <button type="button" class="btn btn-default" onclick="location.href='javascript:history.go(-1);'" style="margin-left:2%">返回</button>
					...
					<input type="hidden" name="requirementEntity.projectId" value="${requirementEntity.projectId}"/>
					<input type="hidden" name="requirementEntity.moduleId" value="${requirementEntity.moduleId}"/>
					<input type="hidden" name="requirementEntity.projectVersionId" value="${requirementEntity.projectVersionId}"/>
					<input type="hidden" name="requirementEntity.source" value="${requirementEntity.source}"/>
					<input type="hidden" name="requirementEntity.priority" value="${requirementEntity.priority}"/>
	...
	<script type="text/javascript">
    	$(function(){
    		...
    		if('${edit}'=='true'){
        		...
        		//回填下拉菜单的选择
        		setSelectedVal('selectProject','${requirementEntity.projectId}');
        		setSelectedVal('selectModule','${requirementEntity.moduleId}');
        		setSelectedVal('selectVersion','${requirementEntity.projectVersionId}');
        		setSelectedVal('selectSourse','${requirementEntity.source}');
        		setSelectedVal('selectPriority','${requirementEntity.priority}');
       ...
       $("#saveRequirementInfo").submit(function(){
		  //验证文件大小，不能超过5M(5*1024*1024)
           ...
           $("input[name='fileDetail']").val(fileNames);
    	 	//将下拉框的选择值填入input框中，以便后台取值
    	   var projectId = $("#selectProject option:selected").val();
    	   $("input[name='requirementEntity.projectId']").val(projectId);
    	   var moduleId = $("#selectModule option:selected").val();
    	   $("input[name='requirementEntity.moduleId']").val(moduleId);
    	   var versionId = $("#selectVersion option:selected").val();
    	   $("input[name='requirementEntity.projectVersionId']").val(versionId);
    	   var source = $("#selectSourse option:selected").val();
    	   $("input[name='requirementEntity.source']").val(source);
    	   var priority = $("#selectPriority option:selected").val();
    	   $("input[name='requirementEntity.priority']").val(priority);
       });
