		...
		success:function(data){
			...
			data.projectChecks.forEach(function(value, index){
				if(index==0){
					...
					$("#modal-content-update").find("input[name='checkItem[0].addTime']").val(value.addTime);
				}else{
					...
					$("#modal-content-update").find("input[name='checkItem["+index+"].addTime']").val(value.addTime);
					index_add++;
				}
			});
...
function saveCheckProject(){
	...
	var templateId = $("input[name='project.templateId']").val();
	...
	$.ajax({
		...
		success:function(data){
			...
			$("input[name='templateId']").each(function(){
				if(templateId == $(this).val()){
					$bodyObj = $(this).parent().next().find(".project");
				}
			});
			var project = data.project;
...
function updateCheckProject(){
	...
	$.ajax({
		...
		success:function(data){
			...
			var $tbodyObj = $obj.parent().parent();
...
function checkInfo(){
	...
	$(".project").each(function(){
		var coefficient = 0;
		$(this).find(".coefficient").each(function(){
...
<body>
	<div class="container-fluid">
      <div class="row">
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
       	  <h3 class="sub-header" style="margin-top:0px;">修改岗位绩效方案</h3> 
       	   <form class="form-horizontal" action="administration/performance/save_updatePerformance" method="post" onsubmit="return checkInfo()">
       	   	   <s:token></s:token>
	 		   ...
	 		   <div style="font-size:15px;margin-left:5%;margin-bottom:5px">
	 		   		<span>岗位：</span><span>${postionPerformance.templateName}</span>
	 		   		...
	 		   		<input type="hidden" name="templateId" value="${postionPerformance.id}">
	 		   </div>
		           	   ...
		           	   <c:forEach items="${postionPerformance.projects}" var="project">
			           	   ...
			           	   <tr>
				  				...
				  				<td class="title">${checkItem.addMoneyType=='+'?'每多':'每少'}</td>
				  			  	...
				  			  	<td class="title">少发金额</td>
				  			  <c:if test="${status.index==0}">
				  			  	<td data-project="${project.id}" data-number="${fn:length(project.checkItems)}"
				  			  	 	rowspan="${fn:length(project.checkItems)*2}">
			  						<a onclick="modifyProject(this)" href="javascript:void(0)">
			        					<svg class="icon" aria-hidden="true" title="修改" data-toggle="tooltip">
			      							<use xlink:href="#icon-modify"></use>
			      						</svg>
	...
	<div id="addCheckProject" class="modal fade" tabindex="-1" role="dialog"
		...
		<div class="modal-dialog" style="width:60%">
			...
			<input type="hidden" name="project.templateId">
		...
		<div class="modal-dialog" style="width:60%">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div style="border-bottom:1px solid #3333333b;margin-bottom:10px"></div>
					<input type="hidden" name="checkItem[0].addTime">
					<div class="form-group">
						<label class="col-sm-2 control-label">考核内容<span style="color:red"> *</span></label>
						<div class="col-sm-3">
							<input class="form-control checkItem" autocomplete="off" required name="checkItem[0].checkItem"/>
