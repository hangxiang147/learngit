            ...
            <table class="table table-striped">
	           <thead>
	           	   <tr>
	           	     ...
	           	     <td style="width:18%">职务</td>
	           	     <td style="width:10%">考核月份</td>
	           ...
	           <tbody>
	           		...
	           		<c:forEach items="${performances}" var="performance" varStatus="status">
	           			<tr>
	           				...
	           				<td>
	           					<c:forEach items="${performance.groupDetailVOs}" var="groupDetailVO">
	           						<li style="list-style:none">${groupDetailVO.companyName}-${groupDetailVO.departmentName}-${groupDetailVO.positionName}</li>
	           					</c:forEach>
	           				</td>
	           				<td>${performance.year}-${performance.month}</td>
	           				<td><fmt:formatDate value="${performance.addTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
	           				<td>${performance.assigneeUserName}</td>
	           				<td>${performance.status}</td>
	           				<td>
	           					<a href="/administration/performance/showPersonalPerformanceDetail?checkItemIds=${performance.checkItemIds}">
			              			<svg class="icon" aria-hidden="true" title="查看方案明细" data-toggle="tooltip">
	            						<use xlink:href="#icon-Detailedinquiry"></use>
	            					</svg>
		              			</a>
		              			&nbsp;
		              			<a href="/administration/performance/showProcessHistory?selectedPanel=updatePerformanceApplys&processInstanceId=${performance.processInstanceID}">
