...
<script type="text/javascript">
	function taskComplete(result) {
		...
		var businessType = "";
		if (businessKey == "Resignation") {
			businessType = "离职申请";
		} 
		var taskID = $("#taskID").val();
		var comment = $("#comment").val();
		var selectedPanel = $("#selectedPanel").val();
		window.location.href = "PM/process/taskComplete?taskID="+taskID+"&result="+result+"&comment="+comment+"&selectedPanel="+selectedPanel+"&businessType="+businessType; 
      ...
      <div class="row">
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
        	<form class="form-horizontal">
        	  <h3 class="sub-header" style="margin-top:0px;"><c:if test="${taskDefKey == 'salarySettlement' }">工资清算确认</c:if></h3>
			  ...
			  <div class="form-group">
			  	<c:if test="${taskDefKey == 'salarySettlement' }">
			  		<button type="button" class="btn btn-primary" onclick="taskComplete(1)" style="margin-left:20px;">通过</button>
			  		<button type="button" class="btn btn-primary" onclick="taskComplete(2)" style="margin-left:20px;">不通过</button>
