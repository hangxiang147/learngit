...
<a href="personal/getVacationAttachmentAll?taskID=${taskId}&index=${index.index}" target="_self"><img style="height:200px;width:200px;margin-left:20px" src="personal/getVacationAttachmentAll?taskID=${taskId}&index=${index.index}"/>
	      	  ...
	      	  <div class="form-group">
        	  		...
        	  		<div class="col-sm-2">
						<input class="form-control"	name="time" autocomplete="off" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"/>
        	  		</div>
			  ...
			  <div class="form-group">
		  		<button type="button" class="btn btn-primary" onclick="taskComplete()" style="margin-left:20px;">确认并通知面试官</button>
  ...
  <script  type="text/html" id="values" >
	...
	function taskComplete(result) {
		...
		if(!time){
			layer.alert("请选择面试时间!",{offset:'100px'},function(){
		...
		var postKey = $("#key").val();
		if(!ids&&!tIds){
			layer.alert("请至少选择一位面试官!",{offset:'100px'});
			return;
		}
		...
		layer.open({
			  ...
			  btn:['确认','取消'],
			  yes:function(){
					window.location.href = "/HR/vitae/completeHrConfirm?taskId="+taskID+"&ids="+ids+"&names="+names+"&tIds="+tIds+"&tNames="+tNames+"&postKey="+postKey+"&time="+time;
			  }
			});
	}
    ...
    $(function (){
 		...
 		(function(textareaSelector,namecySelector,ids,names){
			...
			return arguments.callee;
		})('#agent','#namecy','${ids}','${names}')('#agent_','#namecy_','${t_ids}','${t_names}');
    })
