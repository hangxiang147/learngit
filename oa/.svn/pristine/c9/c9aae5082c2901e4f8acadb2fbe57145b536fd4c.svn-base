...
_extend(KCmd, {
	...
	insertimage : function(url, title, width, height, border, align) {
		...
		var html = '<img onclick="window.parent.showPic(\'\', \'' + _escape(url) + '\')" src="' + _escape(url) + '" data-ke-src="' + _escape(url) + '" ';
		if (width) {
			...
			function dopasteImg() {  
			    ...
			    if (window.clipboardData) {//ie  
			        ...
			        if (clipboardData.files && clipboardData.files.length)//IE11  
			            ...
			            layer.alert("不能粘贴文件或图片,请使用IE11或者Chrome浏览器,或使用上传功能",{offset:'100px'});  
			            return true;  
			        }  
			    } else {  
			    ...
			    if (file) {  
			        if (!K.undef(self.allowImageUpload, true)) {  
			        	layer.alert("编辑器禁止上传图片,请与有关人员联系!",{offset:'100px'});
			            return true;  
			        }  
			        //获取File Blob  
			        var blb;  
			        if (file.getAsFile) {//Chrome  
			            blb = file.getAsFile();  
			            if (blb.size === 0) {  
			            	layer.alert("不能获取剪切板中的" + (file.type.indexOf("image/") === 0 ? "图像" : "文件")  
			                          +"\n如果是从OutLook中复制的,请换其他程序,如Word",{offset:'100px'});
			                return true;  
			            }  
			        ...
			        function sendfile(b, t) {  
			            ...
			            xhr.open('POST', "/administration/notice/attachmentSave");  
			            xhr.onreadystatechange = function () {  
			                if (xhr.readyState == 4&&xhr.status == 200) {  
			                    // if (fn) {  
			                    var data = _trim(xhr.responseText);  
			                    ...
			                    if (data.error) {  
			                        if (typeof ($) !== "undefined" && $.messager && $.messager.alert) {  
			                        	...
			                        	layer.alert(data.message,{offset:'100px'});
			                        }  
			                    } else {  
			                        //self.exec('insertimage', url, title, width, height, border, align);  
			                        if(K.undef(self.formatUploadUrl, true))  
			                            data.url =K.formatUrl(data.url, 'absolute');  
			                        self.exec('insertimage', data.url, "from clipboard", "200", undefined, undefined, undefined);  
			                    }  
			                    //}  
			                    // fn(data);  
			                    // }  
