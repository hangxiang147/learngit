      ...
      <div class="row">
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
        	<form class="form-horizontal">
        	  <h3 class="sub-header" style="margin-top:0px;">报销审批</h3>
...
<%-- 				      	<c:if test="${taskDefKey == 'financialFirstAudit' }">同意</c:if> --%>
				...
				<div class="Cost_foot"><span>会计主管：<c:if test="${taskDefKey ne 'supervisor_audit' and taskDefKey ne 'manager_audit' and taskDefKey ne 'financialFirstAudit'}"><em>${reimbursementVO.showPerson2}</em></c:if></span><span>会计：<em>${reimbursementVO.showPerson1}</em></span><span>出纳：<em>${reimbursementVO.showPerson3}</em></span><span>报销人：<em>${reimbursementVO.requestUserName }</em></span><span>领款人：<em>${reimbursementVO.payeeName }</em></span></div>				
