...
<script type="text/javascript">
	...
	function search() {
		...
		window.location.href = "HR/staff/findStaffAuditList?" + decodeURIComponent(params,true);
		Load.Base.LoadingPic.FullScreenShow(null);
	}
	
	function startAudit(userID) {
		layer.confirm("确认发送背景调查表格给该员工？", {offset:'100px'}, function(index){
			layer.close(index);
			Load.Base.LoadingPic.FullScreenShow(null);
			$.ajax({
				url:'HR/staff/startAudit',
				type:'post',
				data:{userID: userID},
				dataType:'json',
				success:function (data){
					...
					layer.alert("表格已发送，请等待员工填写！", {offset:'100px'}, function(index){
						layer.close(index);
						location.reload();
						Load.Base.LoadingPic.FullScreenShow(null);
					});
      ...
      <div class="row">
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
          <form id="myForm" class="form-horizontal">
          	<h3 class="sub-header" style="margin-top:0px;">背景调查</h3>
          	<div class="form-group">
				<label for="staffName" class="col-sm-1 control-label">姓名</label>
			    <div class="col-sm-2">
			    	<input type="text" class="form-control" id="staffName" name="name" value="${name}" >
			    </div>
			    <label for="status" class="col-sm-1 control-label">调查状态</label>
			    <div class="col-sm-2">
			    	<select class="form-control" id="status" name="auditStatus">
				      <option value="">请选择</option>
					  <option value="0" <c:if test="${not empty auditStatus and auditStatus == 0 }">selected="selected"</c:if>>未调查</option>
					  <option value="1" <c:if test="${auditStatus == 1 }">selected="selected"</c:if>>待员工填写</option>
					  <option value="2" <c:if test="${auditStatus == 2 }">selected="selected"</c:if>>待审核</option>
					  <option value="3" <c:if test="${auditStatus == 3 }">selected="selected"</c:if>>已审核</option>
					  <option value="4" <c:if test="${auditStatus == 4 }">selected="selected"</c:if>>审核未通过</option>
					</select>
			    </div>
			</div>
			<div class="form-group">
            ...
            <table class="table table-striped">
              <thead>
                <tr>
                  ...
                  <th width="120">第一学历</th>
                  <th>专业</th>
                  <th width="20%">毕业学校</th>
                  <th width="120">毕业日期</th>
                  <th width="120">调查状态</th>
                </tr>
              </thead>
              <tbody>
              	...
              	<c:forEach items="${staffVOList}" var="staffVO">
              	    <tr>
              			...
              			<td width="100">${staffVO.lastName}</td>
              			<td>${staffVO.telephone}</td>
              			<td>${staffVO.education}</td>
              			<td>${staffVO.major}</td>
              			<td>${staffVO.school}</td>
              			<td>${staffVO.graduationDate}</td>
              			<td>
              			<c:if test="${staffVO.auditStatus==0 }">
              				<a href="javascript:void(0)" onclick="return startAudit('${staffVO.userID}');">未调查</a>
              			</c:if>
              			<c:if test="${staffVO.auditStatus==1}">待员工填写表格</c:if>
              			<c:if test="${staffVO.auditStatus==2}"><span style="color:yellow;">待审核</span></c:if>              							    
              			<c:if test="${staffVO.auditStatus==3}"><span style="color:green;">已审核</span></c:if>
              			<c:if test="${staffVO.auditStatus==4}"><span style="color:red;">审核未通过</span></c:if>   				    
              			</td>
              		</tr>
